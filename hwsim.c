...
static char *cpath = "etl_config.conf";
module_param(cpath, charp, 0644);

static char *logpath = "";
module_param(logpath, charp, 0644); 
					 ...
					 const u8 *addr, bool add)
{
	...
	if (!msg_head) {
		pr_debug("hwsim: problem with msg_head\n");
...
nla_put_failure:
	nlmsg_free(skb);
err_free_txskb:
	pr_debug("hwsim: error occurred in %s\n", __func__);
...
#define TEST_RADIOTAP_AUTO_FILLED 0
#if (TEST_RADIOTAP_AUTO_FILLED)
static int etl_hwsim_tx_radiotap_len(struct ieee80211_tx_info *info,
                     struct ieee80211_tx_status *status)
{
    int len = sizeof(struct ieee80211_radiotap_header);

    /* IEEE80211_RADIOTAP_RATE rate and flag? */
...
static struct sk_buff *etl_radiotap_auto_filled(struct sk_buff *tx_skb, struct ieee80211_rate *txrate, struct ieee80211_channel *chan,
												struct ieee80211_hw *hw) {
	struct mac80211_hwsim_data *data = hw->priv;
	struct sk_buff *skb;
	...
	rtap_len = etl_hwsim_tx_radiotap_len(info, &status);
	skb = skb_copy_expand(tx_skb, rtap_len, 0, GFP_ATOMIC);
	if (!skb)
		return NULL;
	...
	rthdr->it_present = cpu_to_le32(BIT(IEEE80211_RADIOTAP_TSFT) |			// BIT(0)
									BIT(IEEE80211_RADIOTAP_FLAGS) |			// BIT(1)
									BIT(IEEE80211_RADIOTAP_CHANNEL) |		// BIT(3)
									BIT(IEEE80211_RADIOTAP_TX_FLAGS));		// BIT(16)

	pos = (u8 *)(rthdr + 1);

	// TSFT: 8 Bytes
	put_unaligned_le64(__mac80211_hwsim_get_tsf(data), pos);
	pos += 8;

	// flags: 1 Bytes (do not have any information)
	pos += 1;
	...
	if (info->status.rates[0].idx >= 0 &&
		...
		*pos = txrate->bitrate / 5;
		pos += 1;
	}

	// CHANNEL: 2 Bytes frequency, 2 Bytes flags (no flags information)
	put_unaligned_le16(chan->center_freq, pos);
	pos += 4;

	// TX_FLAGS: 2 Bytes (no information)
	pos += 2;

	if (info->status.rates[0].flags & IEEE80211_TX_RC_MCS) {
		// MCS: 1 Bytes known, 1 Bytes flags, 1Bytes MCS
		...
		/* VHT: 2 Bytes known, 1 Bytes flags, 1 Bytes bandwidth, 1 Bytes*4 mcs_nss[4], 1 Bytes coding
		 * 		1 Bytes group_id, 1 Bytes aid
		 */
		u16 known = hw->radiotap_vht_details &
					(IEEE80211_RADIOTAP_VHT_KNOWN_GI |
					 IEEE80211_RADIOTAP_VHT_KNOWN_BANDWIDTH);

		rthdr->it_present |= cpu_to_le32(BIT(IEEE80211_RADIOTAP_VHT));

		/* requred alignment from rthdr */
...
#endif
static void etl_hwsim_tx(struct ieee80211_hw *hw,
						 struct sk_buff *tx_skb,
						 struct ieee80211_channel *chan)
{
	struct mac80211_hwsim_data *data = hw->priv;
...
#if (TEST_RADIOTAP_AUTO_FILLED)
	rdp.len = 0;
	skb = etl_radiotap_auto_filled(tx_skb, txrate, chan, hw);
	if (!skb)
		return;

	goto SKIP_FILLED_RDP;
#endif

	if (rdp.len) {
		// filled fixed radiotap later
	...
	// TODO: skb filled information check
	//skb->dev = hwsim_mon;
SKIP_FILLED_RDP:
	skb_reset_mac_header(skb);
	skb->ip_summed = CHECKSUM_UNNECESSARY;
...
// @radiolen: used for hwsim_mon mode
static bool mac80211_hwsim_rx_from_etl(struct sk_buff *skb,
					 ...
					 u8 radiolen)
{
	...
	if (!chan) {
		printk(KERN_WARNING "etl: chan is not set, just dropping skb and waiting for config...\n");
	...
	if (mac80211_hwsim_addr_match(data, hdr->addr1) || is_broadcast_ether_addr(hdr->addr1))
		ack = true;
...
void etl_rx_fwdingToHwsim(struct sk_buff *skb, u8 radiolen) {
	struct mac80211_hwsim_data *data;
	bool ack;
	int flag = 0;

	/* Get the AP interface hw data structure */
	spin_lock(&hwsim_radio_lock);
	list_for_each_entry(data, &hwsim_radios, list) {
		if (!strcmp(data->debugfs->d_name.name, "hwsim")) {
			flag = 1;
			break;
		}
	}
	...
	if (!flag) {
		pr_err("Do not get the AP interface %s\n", skb->dev->name);
		kfree_skb(skb);
		return;
	}

	ack = mac80211_hwsim_rx_from_etl(skb, data, radiolen);
}
			      ...
			      struct sk_buff *skb)
{
	...
	/* TODO: Try to reply the application that all transmit is success
	 * Maybe we should think more about how to fill the real situation
	 */
	ack = true;
...
static void hwsim_hx_sta_update(unsigned char *addr, u8 status)
{
	struct sk_buff *skb = NULL;
	struct tlv_sta_info sta = {0};

	// headroom + data + tailroom, need to check the accuracy value
	int headroom = 60, tailroom = 60, data_len = 120;
	int length = headroom + data_len + tailroom;

	sta.tag = cpu_to_le16(HX_CMD_STA_INFO);
	sta.len = cpu_to_le16(sizeof(struct tlv_sta_info));
	ether_addr_copy(sta.addr, addr);
	sta.status = status;

	pr_info("%s: STA address (%pM), status = %d (%s)\n",__func__, sta.addr,
		status, (status == HX_STA_CONNECTED) ? "CONNECT" : "DISCONNECT");
	...
	skb_put_data(skb, &sta, sizeof(sta));
	etl_mcu_skb_send_msg(skb);
}
				  ...
				  struct ieee80211_sta *sta)
{
	...
	hwsim_hx_sta_update(sta->addr, HX_STA_CONNECTED);
	return 0;
}
				     ...
				     struct ieee80211_sta *sta)
{
	...
	hwsim_hx_sta_update(sta->addr, HX_STA_DISCONNECTED);
	return 0;
}
...
static int etl_mcu_send_cmd_ba(struct ieee80211_ampdu_params *params,
								bool enable, bool tx)
{
	struct sk_buff *skb;
	struct tlv_ba ba = {
		.tag = cpu_to_le16(HX_CMD_BA_SET),
		.len = cpu_to_le16(sizeof(struct tlv_ba)),
		.tid = params->tid,
		.ba_type = (tx) ? HX_BA_INITIATOR : HX_BA_RECIPIENT,
		.amsdu = params->amsdu,
		.ba_en = enable,
		.ssn = cpu_to_le16(params->ssn),
		.ba_win_sz = enable ? params->buf_size : 0,
	};
	// headroom + data + tailroom, need to check the accuracy value
	int headroom = 60, tailroom = 60, data_len = 120;
	int length = headroom + data_len + tailroom;

	ether_addr_copy(ba.sta_addr, params->sta->addr);

	printk(KERN_INFO "etl[%s]:\n\t *sta_addr:%pM\n\t *tid:%d\n\t *ba_type:%s\n\t *amsdu:%d\n\t *ba_en:%s\n\t *ssn:%d\n\t *win_sz:%d\n",
					__func__, &ba.sta_addr[0], ba.tid, tx? "INITIATOR" : "RECIPIENT",
					ba.amsdu, ba.ba_en? "ENABLE" : "DISABLE",
					ba.ssn, ba.ba_win_sz);
	skb = alloc_skb(length, GFP_KERNEL);
	if (!skb)
		return -1;
	...
	skb_put_data(skb, &ba, sizeof(ba));
	return etl_mcu_skb_send_msg(skb);
}

int etl_mcu_add_tx_ba(struct ieee80211_ampdu_params *params, bool enable) 
{
	return etl_mcu_send_cmd_ba(params, enable, true);
}

int etl_mcu_add_rx_ba(struct ieee80211_ampdu_params *params, bool enable)
{
	return etl_mcu_send_cmd_ba(params, enable, false);
}
				       ...
				       struct ieee80211_ampdu_params *params)
{
	...
	char *action_str[] = {"RX_START", "RX_STOP", "TX_START", "TX_STOP_CONT",
							"TX_STOP_FLUSH", "TX_STOP_FLUSH_CONT", "TX_OPERATIONAL"};
	int ret = 0;
	u16 tid = params->tid;

	printk(KERN_INFO "etl[%s]: -> AMPDU_%s\n", __func__, action_str[action]);
	switch (action) {
	...
	case IEEE80211_AMPDU_TX_STOP_FLUSH_CONT:
		ret = etl_mcu_add_tx_ba(params, false);
		ieee80211_stop_tx_ba_cb_irqsafe(vif, sta->addr, tid);
		break;
	case IEEE80211_AMPDU_TX_OPERATIONAL:
		ret = etl_mcu_add_tx_ba(params, true);
		break;
	case IEEE80211_AMPDU_RX_START:
		// TODO: rx_aggr_start function
		ret = etl_mcu_add_rx_ba(params, true);
		break;
	case IEEE80211_AMPDU_RX_STOP:
		// TODO: rx_aggr_stop function
		ret = etl_mcu_add_rx_ba(params, false);
		break;
	default:
		return -EOPNOTSUPP;
	}
				    ...
				    struct hwsim_new_radio_params *param)
{
	...
	if (!hw) {
		pr_debug("hwsim: ieee80211_alloc_hw failed\n");
		err = -ENOMEM;
		goto failed;
	}
	...
	if (err != 0) {
		pr_debug("hwsim: device_bind_driver failed (%d)\n",
		       err);
		goto failed_bind;
	}
	...
	for (band = NL80211_BAND_2GHZ; band < NUM_NL80211_BANDS; band++) {
		...
		sband->ht_cap.ampdu_factor = 0x0;
		sband->ht_cap.ampdu_density = 0x6;
		memset(&sband->ht_cap.mcs, 0,
		       sizeof(sband->ht_cap.mcs));
		sband->ht_cap.mcs.rx_mask[0] = 0xff;
		sband->ht_cap.mcs.rx_mask[1] = 0x0;
	...
	if (err < 0) {
		pr_debug("hwsim: ieee80211_register_hw failed (%d)\n",
		       err);
		goto failed_hw;
	}
				      ...
				      struct genl_info *info)
{
	...
	pr_debug("hwsim: received a REGISTER, "
					 ...
					 void *_notify)
{
	...
	if (notify->portid == hwsim_net_get_wmediumd(notify->net)) {
		printk(KERN_INFO "hwsim: wmediumd released netlink"
...
static int __init hwsim_init_netlink(void)
{
	...
	printk(KERN_INFO "hwsim: initializing netlink\n");
...
{
	pr_debug("hwsim: unregister radios\n");
